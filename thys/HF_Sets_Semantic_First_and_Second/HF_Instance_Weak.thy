theory HF_Instance_Weak
  imports Abstract.Abstract_Second_Godel
...
interpretation WRepr_Provability where
      var = "UNIV :: name set"
  and trm = "UNIV :: tm set"
  and fmla = "UNIV :: fm set"
  and num = "{t. ground t}"
...
interpretation Godel_Form where
      var = "UNIV :: name set"
  and trm = "UNIV :: tm set"
  and fmla = "UNIV :: fm set"
  and num = "{t. ground t}"
...
interpretation g2: Godel_Second_Assumptions where
      var = "UNIV :: name set"
  and trm = "UNIV :: tm set"
  and fmla = "UNIV :: fm set"
  and num = "{t. ground t}"
...
theorem Goedel_II: "\<not> {} \<turnstile> Fls \<Longrightarrow> \<not> {} \<turnstile> neg (PfP \<guillemotleft>Fls\<guillemotright>)"
  by (rule g2.godel_second[unfolded consistent_def PP_def PfP_subst subst.simps simp_thms if_True])
...
interpretation g1: Godel_Form_Minimal_Truth_Soundness_HBL1iff_prv_Compl_Pf_Classical where
      var = "UNIV :: name set"
  and trm = "UNIV :: tm set"
  and fmla = "UNIV :: fm set"
  and num = "{t. ground t}"
...
of Godel_Form_Minimal_Truth_Soundness_HBL_iff_Variant_Cls (interpreted above). However, it
requires less infrastructure (no Pf formula).

The definition of isTrue prevents Isabelle from noticing that the locale has already been
interpreted via the above g1 interpretation of Godel_Form_Minimal_Truth_Soundness_HBL_iff_Variant_Cls.
\<close>
