...
text \<open> Generic (meta-)axioms for syntax and substitution.\<close>
...
text \<open>We only assume that the syntax of our logic has notions of variable, term and formula,
...
locale Generic_Syntax =
  ...
  assumes
    ...
    and
    FvarsT[simp,intro]: "\<And>t. t \<in> trm \<Longrightarrow> FvarsT t \<subseteq> var"
    and
    ...
    and
    Fvars[simp,intro]: "\<And>\<phi>. \<phi> \<in> fmla \<Longrightarrow> Fvars \<phi> \<subseteq> var"
    and
...
(* TODO: Properties of inst, based on properties of subst, under
the assumption that the only free variable of \<phi> is xx .  *)
...
lemma Fvars_rawpsubst_su:
  ...
  have 0: "Fvars \<chi> =  Fvars \<phi> - {x} \<union> (if x \<in> Fvars \<phi> then FvarsT t else {})"
    using Cons.prems unfolding \<chi>_def by (auto simp: tx t Fvars_subst)
...
lemma Fvars_rawpsubst:
  ...
  hence 0: "Fvars \<phi> - {x} \<union> FvarsT t - snd ` set txs = Fvars \<phi> - insert x (snd ` set txs) \<union> FvarsT t"
    by auto
  ...
  have 2: "Fvars \<chi> = Fvars \<phi> - {x} \<union> (if x \<in> Fvars \<phi> then FvarsT t else {})"
    using Cons.prems t x unfolding \<chi>_def using Fvars_subst by auto
  show ?case
    unfolding a rawpsubst.simps 1 2 \<chi>_def[symmetric]
    apply (simp add: a \<chi>_def[symmetric] 1 2) apply (rule conjI)
...
proof-
  ...
  have "Fvars \<chi> =
    ...
    unfolding \<chi>_def set_txs apply(rule Fvars_rawpsubst)
    using assms us_facts set_txs apply auto  using inj_Var
    apply (metis Var imageE image_set rev_subsetD set_zip_leftD)
    using FvarsT_Var IntI empty_iff image_iff insert_iff length_map length_map
      list.set_map set_zip_leftD set_zip_rightD snd_set_zip subset_code(1)
    by (smt "00" contra_subsetD inj_Var set_us)
  also have "\<dots> =
    Fvars \<phi> - snd ` set txs \<union>
  ...
  have 1: "Fvars (rawpsubst \<chi> (zip (map fst txs) us)) =
    ...
    unfolding set_us apply (rule Fvars_rawpsubst) using us_facts assms \<chi> apply auto
...
lemma rawpsubst_compose_freshVar:
  ...
  have 2: "u \<in> Fvars (rawpsubst \<phi> (zip (map Var us) (map snd txs))) \<Longrightarrow> False"
    apply(drule in_Fvars_rawpsubstD) using Cons.prems  apply (auto dest!: set_zip_D)
    by (metis FvarsT_Var Int_iff all_not_in_conv contra_subsetD image_iff insert_iff snd_conv us_facts(1)
        us_facts(4))+
  have 3: "\<And> xx tt. xx \<in> FvarsT t \<Longrightarrow> (tt, xx) \<notin> set txs" find_theorems "set _ = _"  nth
  ...
  have "rawpsubst \<phi> (?uxs @ [(Var u, x)]) = rawpsubst (rawpsubst \<phi> ?uxs) [(Var u, x)]"
    apply(rule rawpsubst_compose[symmetric]) using Cons.prems by (auto intro!: rawpsubst dest!: set_zip_D)
  ...
  also have "\<dots> = subst (rawpsubst \<phi> ?uxs) t x"
    apply(rule subst_subst) using Cons 2 by (auto intro!: rawpsubst dest!: set_zip_D)
  also have "\<dots> = rawpsubst \<phi> ((t,x) # ?uxs)"
    apply(rule subst_rawpsubst_fresh_switch) using Cons.prems 3 apply (auto dest!: set_zip_D)
    apply fastforce .
  also have "\<dots> =  rawpsubst \<phi> ([(t,x)] @ ?uxs)" by simp
  also have "\<dots> = rawpsubst (rawpsubst \<phi> [(t,x)]) ?uxs"
    apply(rule rawpsubst_compose[symmetric]) using Cons.prems by (auto dest!: set_zip_D)
  ...
  hence "rawpsubst (rawpsubst \<phi> ?uuxs) ?ttxs = rawpsubst (rawpsubst (rawpsubst \<phi> [(t,x)]) ?uxs) ?tus"
    using 00 by auto
  also have "\<dots> = rawpsubst (rawpsubst \<phi> [(t,x)]) txs" apply(rule Cons.IH)
    using Cons.prems apply (auto intro!: rawpsubst dest!: set_zip_D in_Fvars_substD)
  ...
  also have "\<dots> = rawpsubst \<phi> ([(t,x)] @ txs)"
    apply(rule rawpsubst_compose) using Cons.prems by (auto dest!: set_zip_D)
  finally show ?case by simp
qed auto
...
lemma rawpsubst_compose_freshVar2_aux:
  ...
  have 0: "rawpsubst (rawpsubst ?\<phi>ux (zip (map Var us) xs)) (zip ts us) =
    ...
    apply(rule Cons.IH) using Cons.prems by (auto intro!: rawpsubst dest!: set_zip_D simp: Fvars_subst)

  have 1: "rawpsubst ?\<phi>ux (zip (map Var vs) xs) =
           subst (rawpsubst \<phi> (zip (map Var vs) xs)) (Var u) x"
    apply(rule subst_rawpsubst_fresh_switch[simplified,symmetric])
    using Cons.prems by (auto intro!: rawpsubst dest!: set_zip_D simp: subset_eq)
  ...
  have "subst (subst (rawpsubst \<phi> (zip (map Var vs) xs)) (Var u) x) t u =
    ...
    apply(rule subst_subst)
    using Cons.prems apply (auto intro!: rawpsubst dest!: set_zip_D in_Fvars_rawpsubst_imp simp: Fvars_rawpsubst)
    by force
  also have "\<dots> = subst (subst (rawpsubst \<phi> (zip (map Var vs) xs)) (Var v) x) t v"
    apply(rule subst_subst[symmetric])
    using Cons.prems apply (auto intro!: rawpsubst dest!: set_zip_D in_Fvars_rawpsubst_imp simp: Fvars_rawpsubst)
    by force
  finally have
  ...
  have "rawpsubst (subst (rawpsubst ?\<phi>ux (zip (map Var us) xs)) t u) (zip ts us) =
    ...
    apply(rule subst_rawpsubst_fresh_switch[simplified,symmetric])
    using Cons.prems by (auto intro!: rawpsubst dest!: set_zip_D)
  ...
  also have "\<dots> = rawpsubst (subst (rawpsubst ?\<phi>ux (zip (map Var vs) xs)) t u) (zip ts vs)"
    apply(rule subst_rawpsubst_fresh_switch[simplified])
    using Cons.prems by (auto intro!: rawpsubst dest!: set_zip_D)
...
proof-
  ...
  have ws_facts: "set ws \<subseteq> var"  "distinct ws"
    ...
    using getFrN_Fvars[of "xs @ us @ vs" "ts" "[\<phi>]" _ "length xs"]
      getFrN_FvarsT[of "xs @ us @ vs" "ts" "[\<phi>]" _ "length xs"]
      getFrN_var[of "xs @ us @ vs" "ts" "[\<phi>]" _ "length xs"]
      getFrN_length[of "xs @ us @ vs" "ts" "[\<phi>]" "length xs"]
    apply auto
    apply (metis IntI empty_iff)
    apply (metis IntE IntI distinct_Ex1 empty_iff inf_sup_absorb nth_mem)
    apply (metis IntI Un_iff distinct_Ex1 empty_iff nth_mem)
    by (metis IntI Un_iff distinct_Ex1 empty_iff nth_mem)
...
proof-
  ...
  have us_facts: "set us \<subseteq> var"
    ...
    apply auto
    apply fastforce
    by (metis (no_types, hide_lams) IntI empty_iff image_iff snd_conv)
  ...
  have vs_facts: "set vs \<subseteq> var"
    ...
    using getFrN_FvarsT[of "map snd txs" "map fst txs" "[subst \<phi> s y]" _ "length txs"]
      getFrN_Fvars[of "map snd txs" "map fst txs" "[subst \<phi> s y]" _ "length txs"]
      getFrN_var[of "map snd txs" "map fst txs" "[subst \<phi> s y]" _ "length txs"]
      getFrN_length[of "map snd txs" "map fst txs" "[subst \<phi> s y]" "length txs"]
      getFrN_length[of "map snd txs" "map fst txs" "[subst \<phi> s y]" "length txs"]
    apply auto
    apply fastforce
    by (metis (no_types, hide_lams) IntI empty_iff image_iff snd_conv)
  ...
  have ws_facts: "set ws \<subseteq> var"
    ...
    using getFrN_FvarsT[of "y # map snd txs" "s # map fst txs" "[\<phi>]" _ "length txs"]
      getFrN_Fvars[of "y # map snd txs" "s # map fst txs" "[\<phi>]" _ "length txs"]
      getFrN_var[of "y # map snd txs" "s # map fst txs" "[\<phi>]" _ "length txs"]
      getFrN_length[of "y # map snd txs" "s # map fst txs" "[\<phi>]" "length txs"]
      getFrN_length[of "y # map snd txs" "s # map fst txs" "[\<phi>]" "length txs"]
    apply (auto simp: image_iff)
    apply fastforce using  IntI empty_iff image_iff defer
    apply (metis (no_types, hide_lams) snd_conv)
  proof -
    fix x :: 'var and a :: 'trm and b :: 'var
    assume a1: "\<And>t. t = s \<or> (\<exists>x\<in>set txs. t = fst x) \<Longrightarrow>
     set (getFrN (y # map snd txs) (s # map fst txs) [\<phi>] (length txs)) \<inter> FvarsT t = {}"
    assume a2: "(a, b) \<in> set txs"
    assume a3: "x \<in> FvarsT a"
    assume "x \<in> set (getFrN (y # map snd txs) (s # map fst txs) [\<phi>] (length txs))"
    then show False
      using a3 a2 a1 by (metis (no_types) IntI empty_iff fst_conv)
  qed
  ...
  have 0: "rawpsubst (subst \<phi> s y) ?wxs = subst (rawpsubst \<phi> ?wxs) s y"
    apply(subst rawpsubst_compose[of \<phi> ?wxs  "[(s,y)]",simplified]) using assms ws_facts
    apply (auto dest!: set_zip_D simp: subset_eq)
    apply(subst rawpsubst_subst_fresh_switch) using ws_facts assms
    by (auto dest!: set_zip_D simp: subset_eq)

  have 1: "rawpsubst (rawpsubst \<phi> ?wxs) ?tws = rawpsubst (rawpsubst \<phi> ?uxs) ?tus"
    apply(rule rawpsubst_compose_freshVar2) using assms ws_facts us_facts by (auto simp: subset_eq Fvars_subst)

  have "rawpsubst (rawpsubst (subst \<phi> s y) ?vxs) ?tvs =
        rawpsubst (rawpsubst (subst \<phi> s y) ?wxs) ?tws"
    apply(rule rawpsubst_compose_freshVar2) using assms ws_facts vs_facts by (auto simp: subset_eq Fvars_subst)
  ...
  also have "\<dots> = subst (rawpsubst (rawpsubst \<phi> ?wxs) ?tws) s y"
    apply(subst rawpsubst_compose[of "rawpsubst \<phi> ?wxs" ?tws  "[(s,y)]",simplified]) using assms ws_facts
    apply (auto dest!: set_zip_D simp: subset_eq intro!: rawpsubst)
    apply(subst rawpsubst_subst_fresh_switch) using ws_facts assms
    by (auto dest!: set_zip_D simp: subset_eq intro!: rawpsubst)
...
proof-
  have "psubst \<phi> (map (\<lambda>x. (Var x,x)) xs) = rawpsubst \<phi> (map (\<lambda>x. (Var x,x)) xs)"
    apply(rule psubst_eq_rawpsubst) using assms apply (auto simp: o_def)
    by (metis FvarsT_Var contra_subsetD distinct_Ex1 empty_iff insert_iff
        less_Suc_eq less_trans_Suc not_less_eq nth_mem)
...
proof-
  ...
  have [simp]: "rawpsubst \<phi> (zip (map Var us) (map snd txs)) = \<phi>"
    apply(rule rawpsubst_notIn)
    using assms us_facts apply auto
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast
    apply(drule set_zip_rightD) apply simp apply blast .
  have [simp]: "rawpsubst \<phi> (zip (map fst txs) us) = \<phi>"
    ...
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast
    apply(drule set_zip_rightD) by auto
  show ?thesis using assms us_facts unfolding psubst_def
    by(auto simp: Let_def us[symmetric])
qed


end \<comment> \<open>context Generic_Syntax\<close>
...
locale Syntax_with_Numerals =
  ...
  for var :: "'var set" and trm :: "'trm set" and fmla :: "'fmla set"
    ...
    +
  fixes
    (* Abstract notion of numerals, as a subset of the ground numeric trms: *)
    num :: "'trm set"
  assumes
    numNE: "num \<noteq> {}"
    and
...
proof-
  ...
  have t: "?t = n" apply(rule rawpsubstT_num) using assms us_facts apply auto
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast .
  have "rawpsubstT ?t (zip (map fst txs) us) = n" unfolding t
    apply(rule rawpsubstT_num) using assms us_facts apply auto
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast .
  thus ?thesis unfolding psubstT_def by(simp add: Let_def us[symmetric])
qed

end \<comment> \<open>Syntax_with_Numerals\<close>
...
locale Syntax_with_Connectives =
  ...
  assumes
    ...
    and
    subst_eql[simp]:
    "\<And> t1 t2 t x. t \<in> trm \<Longrightarrow> t \<in> trm \<Longrightarrow> t2 \<in> trm \<Longrightarrow> x \<in> var \<Longrightarrow>
...
proof-
  ...
  have c: "?c = cnj (rawpsubst \<phi>1 ?uxs) (rawpsubst \<phi>2 ?uxs)"
    apply(rule rawpsubst_cnj) using assms us_facts apply (auto intro!: rawpsubstT)
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast .
  have 0: "rawpsubst ?c ?tus =
    cnj (rawpsubst (rawpsubst \<phi>1 ?uxs) ?tus) (rawpsubst (rawpsubst \<phi>2 ?uxs) ?tus)"
    unfolding c
    apply(rule rawpsubst_cnj)
    using assms us_facts apply (auto intro!: rawpsubst)
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast
  ...
  have 1: "rawpsubst (rawpsubst \<phi>1 ?uxs) ?tus = rawpsubst (rawpsubst \<phi>1 ?vxs1) ?tvs1"
    ...
    using assms vs1_facts us_facts by (auto intro!: rawpsubst)
  have 2: "rawpsubst (rawpsubst \<phi>2 ?uxs) ?tus = rawpsubst (rawpsubst \<phi>2 ?vxs2) ?tvs2"
    apply(rule rawpsubst_compose_freshVar2)
    using assms vs2_facts us_facts by (auto intro!: rawpsubst)
...
lemma rawpsubst_imp:
  ...
  using assms apply (induct txs arbitrary: \<phi>1 \<phi>2) apply auto[]
...
proof-
  ...
  have c: "?c = eqv (rawpsubst \<phi>1 ?uxs) (rawpsubst \<phi>2 ?uxs)"
    apply(rule rawpsubst_eqv) using assms us_facts apply (auto intro!: rawpsubstT)
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast .
  have 0: "rawpsubst ?c ?tus =
    eqv (rawpsubst (rawpsubst \<phi>1 ?uxs) ?tus) (rawpsubst (rawpsubst \<phi>2 ?uxs) ?tus)"
    unfolding c
    apply(rule rawpsubst_eqv)
    using assms us_facts apply (auto intro!: rawpsubst)
    apply(drule set_zip_rightD) apply simp apply blast
    apply(drule set_zip_leftD) apply simp apply blast
    apply(drule set_zip_rightD) apply simp apply blast
...
lemma rawpsubst_all:
  ...
  using assms apply (induct txs arbitrary: \<phi>) apply auto[]
...
proof-
  ...
  have 0: "rawpsubst (all y \<phi>) (zip (map Var ws) (map snd txs)) =
    ...
    apply(rule rawpsubst_all) using assms ws_facts apply (auto dest!: set_zip_D) by auto
  ...
  have "rawpsubst (rawpsubst (all y \<phi>) (zip (map Var us) (map snd txs))) (zip (map fst txs) us) =
    ...
    apply(rule rawpsubst_compose_freshVar2)
    using assms ws_facts us_facts by (auto intro!: rawpsubst)
  also have
    "\<dots> = all y (rawpsubst ((rawpsubst \<phi> (zip (map Var ws) (map snd txs)))) (zip (map fst txs) ws))"
    unfolding 0 apply(rule rawpsubst_all)  using assms ws_facts by (auto dest!: set_zip_D intro!: rawpsubst)
  also have
...
proof-
  ...
  have 0: "rawpsubst (exi y \<phi>) (zip (map Var ws) (map snd txs)) =
    ...
    apply(rule rawpsubst_exi) using assms ws_facts apply (auto dest!: set_zip_D) by auto
  ...
  also have
    ...
    unfolding 0 apply(rule rawpsubst_exi)  using assms ws_facts by (auto dest!: set_zip_D intro!: rawpsubst)
  also have
...
end \<comment> \<open>context Syntax_with_Connectives\<close>
...
(* The "soft substitution" function (whose representability is used as an alternative
to real substitution for the diagonal lemma: *)
...
(* Negation as a derrived connective: *)
...
(* Parallel substitution versus the new connectives: *)
...
proof-
  ...
  have [simp]: "rawpsubst fls (zip (map Var us) (map snd txs)) = fls"
    apply(rule rawpsubst_fls) using us_facts assms by (auto dest!: set_zip_D)
...
end \<comment> \<open>context Syntax_with_Connectives_False\<close>


(* The addition of disjunction among logical connectives.
...
end \<comment> \<open>context Syntax_with_Connectives_False_Disj\<close>
...
Fvars (LLq t1 t2) = FvarsT t1 \<union> FvarsT t2"
  by (auto simp add: Fvars_subst LLq_def2 subst2_fresh_switch)
